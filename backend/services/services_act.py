...
def get_act_by_id(act_id: int):
    act = get_supabase_client().table("acts").select("*").eq("id", act_id).single().execute()
    if act:
        return act
    return {"message": "Act not found"}

def get_all_acts():
    acts = get_supabase_client().table("acts").select("*").execute()
    if acts:
        return acts
    return {"message": "No acts found"}

def get_act_by_name(act_name: str):
    act = get_supabase_client().table("acts").select("*").ilike("title", f"%{act_name}%").execute()
    if act:
        return act
    return {"message": "Act not found with the given name"}

def search_acts_by_keyword(keyword: str):
    acts = get_supabase_client().table("acts").select("*").ilike("preface", f"%{keyword}%").execute()
    if acts:
        return acts
    return {"message": "No acts found matching the keyword"}

def get_act_books(act_id: int):
    books = get_supabase_client().table("act_books").select("*").eq("act_id", act_id).execute()
    if books:
        return books
    return {"message": "No books found for this act"}

def get_act_book_by_id(book_id: int):
    book = get_supabase_client().table("act_books").select("*").eq("id", book_id).single().execute()
    if book:
        return book
    return {"message": "Book not found"}

def get_act_book_by_act_and_book_number(act_id: int, book_number: str):
    book = get_supabase_client().table("act_books").select("*").eq("act_id", act_id).eq("book_number", book_number).single().execute()
    if book:
        return book
    return {"message": "Book not found with the given act ID and book number"}

def search_books_by_keyword(keyword: str):
    books = get_supabase_client().table("act_books").select("*").ilike("title", f"%{keyword}%").execute()
    if books:
        return books
    return {"message": "No books found matching the keyword"}

def get_act_groups(book_id: int):
    groups = get_supabase_client().table("act_groups").select("*").eq("book_id", book_id).execute()
    if groups:
        return groups
    return {"message": "No groups found for this book"}

def get_act_group_by_id(group_id: int):
    group = get_supabase_client().table("act_groups").select("*").eq("id", group_id).single().execute()
    if group:
        return group
...
def get_act_group_by_act_and_group_number(act_id: int, group_number: str):
    group = get_supabase_client().table("act_groups").select("*").eq("act_id", act_id).eq("group_number", group_number).single().execute()
    if group:
        return group
    return {"message": "Group not found with the given act ID and group number"}

def search_groups_by_keyword(keyword: str):
    groups = get_supabase_client().table("act_groups").select("*").ilike("title", f"%{keyword}%").execute()
    if groups:
        return groups
...
def get_act_super_section_by_id(super_section_id: int):
    super_section = get_supabase_client().table("act_super_sections").select("*").eq("id", super_section_id).single().execute()
    if super_section:
...
def search_super_sections_by_keyword(keyword: str):
    super_sections = get_supabase_client().table("act_super_sections").select("*").ilike("title", f"%{keyword}%").execute()
    if super_sections:
        return super_sections
